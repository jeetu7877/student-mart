...
// NEW: Function to upload image to backend
...
// MODIFIED: Add product function to support backend image upload
async function addProduct() {
    ...
    const uploadPromises = Array.from(imageInput.files).map(uploadImageToServer);
    const images = await Promise.all(uploadPromises);

    if (images.includes(null)) {
        showAlert('One or more images failed to upload.', 'error');
        return;
    }
...
// Add to cart
function addToCart(productId, showAlertMessage = false) {
    ...
    // ✅ Quantity check (if quantity input doesn't exist, default to 1)
...
function openCart() {
    showPage('cart');  // Make sure your cart page has id="cart"
    renderCart();       // Load current cart items
}
